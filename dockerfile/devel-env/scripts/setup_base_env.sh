...
function setup_ssh()
{
    local workdir=$1
    mkdir -p ~/.ssh
    cp $workdir/configs/ssh_config ~/.ssh/config
    cp $workdir/configs/ssh_id_rsa ~/.ssh/id_rsa
    cp $workdir/configs/ssh_id_rsa.pub ~/.ssh/id_rsa.pub
}
...
{
    local workdir=$1
    local cfg_dir=$workdir/configs
    local pkg_dir=$workdir/packages
    
    # neovim基础软件已安装
    mkdir -p ~/.config/nvim
    \cp $cfg_dir/init.vim ~/.config/nvim/
    \cp $cfg_dir/coc-settings.json ~/.config/nvim/

    # 安装插件
    pushd $pkg_dir > /dev/null 2>&1
    # 安装vim-plug插件
    tar xzvf vim-plug-0.11.0.tar.gz
    mkdir -p ~/.config/nvim/autoload
    \cp vim-plug-0.11.0/plug.vim ~/.config/nvim/autoload/
    
    # 安装其它插件
    tar xzvf nvim-plugged.tar.gz
    mkdir -p ~/.local/share/nvim/
    mv plugged  ~/.local/share/nvim/
    popd > /dev/null 2>&1
    
    # 配置coc插件
    pushd ~/.local/share/nvim/plugged/coc.nvim > /dev/null 2>&1
    yarn install
    popd > /dev/null 2>&1
}
...
# #######################################################
TMP_WORKDIR=$1
setup_ssh $TMP_WORKDIR
setup_git
setup_github
#setup_fzf $TMP_WORKDIR
setup_neovim $TMP_WORKDIR
